...
const QueryTab: React.FC<QueryTabProps> = ({
  ...
  <button
    onClick={() => onClick(id)}
    className={cn(
      "rounded-full bg-white text-[14px] leading-[20px] px-4 py-2 w-1/2 flex gap-2 justify-center cursor-default",
      isActive && "scale-105 transition duration-200 shadow-effect-tabs",
      id === 1
        ? "rounded-l-full border-r border-light-gray rounded-r-none"
        : "rounded-r-full border-l border-light-gray rounded-l-none"
    )}
  >
    {icon}
    {label}
...
const QuerySection: React.FC = () => {
  ...
  const renderQueryContent = () => {
    if (queryId === 1) {
      ...
      return <Image src={Visual} alt="visual" width={816} height={650} />;
    }
  };

  const queryTabs: QueryTabData[] = [
    {
      id: 1,
      icon: (
        <svg
